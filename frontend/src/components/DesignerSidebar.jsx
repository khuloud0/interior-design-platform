...
import { LayoutDashboard, ClipboardList, Settings, User, LogOut } from "lucide-react";
...
export default function DesignerSidebar({ variant = "light" }) {
  ...
  const navItems = [
    { icon: <LayoutDashboard size={18} strokeWidth={1.5} />, label: "Avaliable Request", path: "/designer/dashboard"  },
    { icon: <ClipboardList   size={18} strokeWidth={1.5} />, label: "Manage Project",    path: "/designer/manage"     },
    { icon: <User            size={18} strokeWidth={1.5} />, label: "Edit Profile",      path: "/designer/edit-profile" },
    { icon: <Settings        size={18} strokeWidth={1.5} />, label: "Setting",           path: "/designer/settings"   },
  ];

  const getIsActive = (itemPath) => {
    const current = location.pathname;
    if (current === itemPath) return true;
    if (itemPath === "/designer/manage" && current.endsWith("/create-plan")) return true;
    if (itemPath === "/designer/dashboard" && current.startsWith("/designer/requests") && !current.endsWith("/create-plan")) return true;
    if (itemPath === "/designer/edit-profile" && (current === "/designer/profile" || current === "/designer/MyProfile")) return true;
    return false;
  };
