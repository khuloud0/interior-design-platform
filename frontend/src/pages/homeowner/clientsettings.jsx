...
import { User, Lock, Bell, CreditCard, Check, Eye, EyeOff } from "lucide-react";
...
const ComingSoon = ({ children }) => (
  <div style={{ position: "relative" }}>
    ...
    <div style={{ position: "absolute", inset: 0, display: "flex", alignItems: "center", justifyContent: "center" }}>
      <div style={{ background: C.card, border: `0.5px solid ${C.border}`, borderRadius: 14, padding: "20px 36px", textAlign: "center", boxShadow: "0 8px 32px rgba(44,34,26,0.10)" }}>
        <div style={{ fontSize: 18, marginBottom: 6 }}>🔒</div>
        <div style={{ fontFamily: f.serif, fontSize: 22, fontWeight: 400, color: C.dark, marginBottom: 4 }}>Coming Soon</div>
...
const Toggle = ({ checked, onChange }) => (
  <button onClick={() => onChange(!checked)}
    style={{ width: 40, height: 22, borderRadius: 11, border: "none", cursor: "pointer", background: checked ? C.dark : C.border, transition: "background .2s", position: "relative", flexShrink: 0 }}>
    <span style={{ position: "absolute", top: 3, left: checked ? 20 : 3, width: 16, height: 16, borderRadius: "50%", background: C.card, transition: "left .2s" }} />
  </button>
);
...
const SaveBtn = ({ onClick, saving, saved }) => (
  <button onClick={onClick} disabled={saving}
    style={{ padding: "10px 28px", border: "none", borderRadius: 8, background: saved ? C.success : C.dark, color: C.sand, fontSize: 11, fontWeight: 500, fontFamily: f.font, letterSpacing: ".12em", textTransform: "uppercase", cursor: saving ? "not-allowed" : "pointer", opacity: saving ? 0.7 : 1, transition: "all .2s", display: "flex", alignItems: "center", gap: 8 }}>
    {saved ? <><Check size={12} strokeWidth={2} /> Saved</> : saving ? "Saving..." : "Save Changes"}
  </button>
);

const Toast = ({ msg, type }) => (
  <div style={{ position: "fixed", bottom: 32, left: "50%", transform: "translateX(-50%)", background: type === "error" ? C.error : C.dark, color: C.sand, padding: "12px 28px", borderRadius: 8, fontSize: 12, fontFamily: f.font, zIndex: 999, letterSpacing: "0.04em", display: "flex", alignItems: "center", gap: 8 }}>
...
function AccountTab() {
  ...
  const [form, setForm]     = useState({ name: client.name || "", email: client.email || "", phone: client.phone || "", city: client.city || "" });
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (res.ok) {
        localStorage.setItem("user", JSON.stringify({ ...client, ...form }));
        setSaved(true);
...
function NotificationsTab() {
  ...
  const handleSave = async () => {
    ...
    try {
      const token = localStorage.getItem("token");
      const res = await fetch("https://interior-design-platform-production.up.railway.app/client/notifications/settings", {
        method: "PATCH",
        headers: { "Content-Type": "application/json", Authorization: `Bearer ${token}` },
  ...
  return (
    <>
      <SectionCard>
        ...
        <NotifRow label="Request Updates"      desc="Updates on your submitted design requests"       checked={notifs.request_update}   onChange={v => set("request_update", v)} />
        <NotifRow label="Offer Received"       desc="When a contractor submits an offer on your request" checked={notifs.offer_received}   onChange={v => set("offer_received", v)} />
        <NotifRow label="Project Started"      desc="When a project moves to active status"           checked={notifs.project_started}  onChange={v => set("project_started", v)} />
        <NotifRow label="Project Completed"    desc="When your project is marked as complete"         checked={notifs.project_complete} onChange={v => set("project_complete", v)} />
        <NotifRow label="Payment Sent"         desc="Confirmation when a payment is processed"        checked={notifs.payment_sent}     onChange={v => set("payment_sent", v)} />
        <NotifRow label="Marketing & Updates"  desc="Tips, offers, and platform news"                 checked={notifs.marketing}        onChange={v => set("marketing", v)} />
        <div style={{ marginTop: 24, display: "flex", justifyContent: "flex-end" }}>
          <SaveBtn onClick={handleSave} saving={saving} saved={saved} />
        </div>
      </SectionCard>
...
function BillingTab() {
  const [invoices] = useState([
    { id: "INV-0021", date: "May 1, 2025",  amount: 8500, status: "paid" },
    { id: "INV-0018", date: "Apr 3, 2025",  amount: 4200, status: "paid" },
    { id: "INV-0014", date: "Mar 10, 2025", amount: 6750, status: "paid" },
  ]);
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
      <SectionCard>
        ...
        <div style={{ padding: "20px 0", textAlign: "center", color: C.muted, fontSize: 13, fontWeight: 300, lineHeight: 1.7 }}>
          Payment methods will be available soon.<br />
          <span style={{ fontSize: 11 }}>All transactions are currently handled offline.</span>
        </div>
      </SectionCard>
      <SectionCard>
