...
// ✅ ProviderRow — زر Select يفتح الـ Payment
...
function RequestDetails({ request = {}, designVision = null, offers = [], clientAttachments = [], onEdit, onDesignerClick, onSelectOffer, selectingOffer, selectedOfferId }) {
  const {
    ...
    space_size, desired_start, duration, city, country,
  } = request;
        ...
        <Section label="Request details">
          <div style={{ background: "#EDE5DC", borderRadius: 16, border: `0.5px solid ${C.border}`, overflow: "hidden" }}>
            {[
              ...
              { icon: <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.4" strokeLinecap="round" strokeLinejoin="round"><path d="M12 21s7-6.5 7-12A7 7 0 0 0 5 9c0 5.5 7 12 7 12Z"/><circle cx="12" cy="9" r="2.4"/></svg>, label: "LOCATION", value: locationValue },
              ...
              <div style={{ padding: "28px 24px" }}>
                ...
                {(designVision.materials || designVision.colors) && (
                  <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 14, marginBottom: 24 }}>
                    {designVision.materials && <div><div style={{ ...T.label, marginBottom: 6 }}>Materials</div><div style={{ fontSize: 13, color: C.dark }}>{designVision.materials}</div></div>}
                    {designVision.colors && <div><div style={{ ...T.label, marginBottom: 6 }}>Color Palette</div><div style={{ fontSize: 13, color: C.dark }}>{designVision.colors}</div></div>}
                  </div>
                )}
...
export function RequestDetailsPage() {
  ...
  // ✅ يفتح الـ Payment Modal بدل ما يرسل مباشرة
  const handleSelectOffer = (offer) => {
    setPayOffer(offer);
    setPayOpen(true);
  };

  // ✅ بعد الدفع — يحدث الـ status لـ completed
  const handlePaymentComplete = async () => {
    ...
    try {
      ...
      setSelectedOfferId(payOffer.id);
    } catch {}
    finally {
    ...
    <>
      ...
      {payOpen && (
        <PaymentFlow
          offer={{
            ...payOffer,
            // ✅ المبلغ الإجمالي من الطلب نفسه
            price: payOffer?.price || data.request?.budget || 0,
          }}
