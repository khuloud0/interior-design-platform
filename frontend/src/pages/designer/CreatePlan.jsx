  ...
  const processFiles = async (files) => {
    ...
    for (const file of files) {
      ...
      if (uploadFile.size > MAX_SIZE) {
        newItems.push({ file: uploadFile, name: uploadFile.name, size: uploadFile.size, type: uploadFile.type, preview: null, path: null, uploading: false, uploaded: false, error: "File exceeds 20MB limit." });
        continue;
      }
      const preview = uploadFile.type.startsWith("image/") ? URL.createObjectURL(uploadFile) : null;
      newItems.push({ file: uploadFile, name: uploadFile.name, size: uploadFile.size, type: uploadFile.type, preview, path: null, uploading: false, uploaded: false, error: null });
    }
    setAttachments(prev => {
                  ...
                  <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
                    {(() => {
                      ...
                      return totalSize > 5 * 1024 * 1024 ? (
                        <div style={{ fontSize: 11, color: C.accent, background: "rgba(201,144,42,0.08)", padding: "8px 12px", borderRadius: 8, display: "flex", alignItems: "center", gap: 6 }}>
                          Total size: {formatFileSize(totalSize)} — Large files may take longer to upload.
                        </div>
                      ) : null;
                    })()}
