...
function AccountTab() {
  ...
  const [form, setForm]     = useState({ name: designer.name || "", email: designer.email || "", phone: designer.phone || "", city: designer.city || "" });
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (res.ok) {
        localStorage.setItem("user", JSON.stringify({ ...designer, ...form }));
        setSaved(true);
...
function NotificationsTab() {
  ...
  const handleSave = async () => {
    ...
    try {
      const token = localStorage.getItem("token");
      const res = await fetch("http://127.0.0.1:5000/designer/notifications/settings", {
        method: "PATCH",
        headers: { "Content-Type": "application/json", Authorization: `Bearer ${token}` },
  ...
  return (
    <>
      <SectionCard>
        ...
        <NotifRow label="New Request"          desc="When a client submits a new design request"        checked={notifs.new_request}      onChange={v => set("new_request", v)} />
        <NotifRow label="Request Accepted"     desc="When a client accepts your proposal"               checked={notifs.request_accepted} onChange={v => set("request_accepted", v)} />
        <NotifRow label="Request Declined"     desc="When a client declines your proposal"              checked={notifs.request_declined} onChange={v => set("request_declined", v)} />
...
function BillingTab() {
  const [invoices] = useState([
    { id: "INV-0033", date: "May 1, 2025",  amount: 3200, status: "paid" },
    { id: "INV-0029", date: "Apr 5, 2025",  amount: 5800, status: "paid" },
    { id: "INV-0024", date: "Mar 12, 2025", amount: 2400, status: "paid" },
  ]);
  ...
  const handleSave = async () => {
    ...
    try {
      const token = localStorage.getItem("token");
      const res = await fetch("http://127.0.0.1:5000/designer/billing", {
        method: "PATCH",
        headers: { "Content-Type": "application/json", Authorization: `Bearer ${token}` },
